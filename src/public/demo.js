...
window.fetch = async function(url, options) {
    ...
    try {
        ...
        // If we get a network/CORS error, fall back to mock data
        ...
        if (url.includes('/api/products')) {
            ...
            const skinType = queryParams.get('skinType') || 'all';
            
            let filtered = products;
            if (category !== 'all') filtered = filtered.filter(p => p.category === category);
            if (skinType !== 'all') filtered = filtered.filter(p => p.skinType.includes(skinType));
...
class ReviewSystem {
    ...
    async submitReview() {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/products/${this.currentProductId}/reviews`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
async function showEnhancedProductDetail(product) {
    ...
    // ALWAYS use productId or id (numeric) - no MongoDB _id
    ...
    // Get detailed product info from backend
    let detailedProduct = product;
    
    // Only try to fetch from backend if we have a valid NUMERIC product ID
    const shouldFetchFromBackend = productIdForAPI && 
                                  !isNaN(productIdForAPI) && 
                                  productIdForAPI > 0;
    
    if (shouldFetchFromBackend) {
        try {
            console.log(`🔍 Fetching product details for numeric ID: ${productIdForAPI}`);
            const response = await fetch(`${API_BASE_URL}/products/${productIdForAPI}`);
            ...
            if (response.ok) {
                ...
                if (data.success && data.product) {
                    ...
                    console.log('⚠️ Backend returned no product data, using local');
                }
            } else {
                console.log('⚠️ Backend request failed, using local product');
            }
        } catch (error) {
            console.log('❌ Error fetching from backend, using local:', error.message);
        }
    } else {
        console.log('🔄 Using local product data only');
    }

    // Get reviews
    const reviews = await reviewSystem.getProductReviews(productIdForAPI || product.id);
    ...
    const reviewsSection = reviews.length > 0 ? `
        <div class="product-reviews" style="margin-top: 30px; padding: 20px; background: #f5f5f5; border-radius: 10px;">
            <div class="review-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <button class="write-review-btn" data-product-id="${productIdForAPI || product.id}" style="
                    background: #4a7c59;
                    color: white;
                    border: none;
                    padding: 10px 20px;
            ...
            <div class="review-list" style="max-height: 200px; overflow-y: auto;">
                ${reviews.slice(0, 3).map(review => `
                    <div class="review-item" style="background: white; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                        <div class="review-meta" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                            <span class="review-author" style="font-weight: 600; color: #333;">${review.userName || 'Customer'}</span>
                        ...
                        <div class="review-text" style="line-height: 1.6; color: #555;">${review.text}</div>
                    </div>
                `).join('')}
            </div>
        </div>
                ...
                ">
                    <div>
                        ...
                        ">
                            ...
                            <span style="color: #6b7280;">(${detailedProduct.numReviews || detailedProduct.reviewCount || reviews.length} reviews)</span>
                        </div>
                ...
                ${order.orderItems && order.orderItems.map(item => `
                    <div style="display: flex; align-items: center; padding: 10px 0; border-bottom: 1px solid #eee;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <p style="margin: 0; color: #666; font-size: 12px;">Qty: ${item.quantity || 1}</p>
                        </div>
...
class StripePayment {
    ...
    loadStripeScript() {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => {
                ...
                simpleNotification.show('Payment system failed to load. Please refresh.', 'error');
    ...
    async handlePaymentWithCard(event) {
        ...
        try {
            ...
            emailNotifications.showOrderConfirmation(this.orderId);
    ...
    async syncProducts(filters = {}) {
        ...
        try {
            ...
            if (data.success && Array.isArray(data.data)) {
                ...
                // Ensure all products have numeric IDs
    ...
    async syncFeaturedProducts() {
        ...
        try {
            ...
            if (data.success && Array.isArray(data.data) && data.data.length > 0) {
                ...
                // Ensure all featured products have numeric IDs
                return data.data.map(product => ({
                    ...product,
                    id: product.productId || product.id
                }));
...
// ==================== UPDATED: FIXED attachProductEventListeners FUNCTION ====================
function attachProductEventListeners() {
    ...
    setTimeout(() => {
        ...
        // **FIXED: Better event delegation that doesn't interfere with button clicks**
        document.body.addEventListener('click', function handleProductCardClick(e) {
            const card = e.target.closest('.product-card');
            
            if (!card) return;
            
            // **CHANGED: Only prevent card click if clicking on specific buttons**
            ...
            // **FIXED: Check if click is on wishlist button**
...
function initializeProductCards() {
    ...
    setTimeout(() => {
        ...
        attachAddToCartListeners(); // **ADD THIS LINE**
...
// Emergency fix: Prevent API calls with undefined IDs
const originalShowEnhancedProductDetail = showEnhancedProductDetail;

showEnhancedProductDetail = async function(product) {
    // Don't call API if product ID is invalid
    if (!product.id || product.id === 'undefined' || product.id === 'null') {
        console.log('🛑 Skipping API call for invalid product ID:', product.id);
        // Create modal using only local data
        createSimpleProductModal(product);
        return;
    }
    
    // Original function
    return originalShowEnhancedProductDetail.call(this, product);
};

function createSimpleProductModal(product) {
    // Create a basic modal without API calls
    const modal = document.createElement('div');
    modal.id = 'productDetailModal';
    modal.className = 'modal';
    modal.style.cssText = `
        ...
        ">
            ...
            <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                ...
                <div style="flex: 2;">
                    ...
                    <div style="font-size: 24px; font-weight: bold; color: #4a7c59; margin-bottom: 20px;">
    ...
    addToCartBtn.addEventListener('click', function() {
        ...
        addToCart(id, name, price, image, 10);
        simpleNotification.show(`Added ${name} to cart!`, 'success');
